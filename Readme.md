...
*Note that how I do it here is not a memory safe way and if you have a
very fine resolution raster with many species, it may cause issues with
your R session. If you really struggle here you may write me a mail to a
find a better solution.*

``` r
# Transform the species stack into a matrix
salticidae.matrix <- getValues(sp.stack)

# Add cell numbers as row names
rownames(salticidae.matrix) <- 1:nrow(salticidae.matrix)
